...
for d in "${degrees[@]}"; do
    for s in "${score_thres[@]}"; do
        for di in "${divider[@]}"; do 
            python -m tracking.main track --dataset_path=$1 --improved=True --degree_thres=$d --score_thres=$s --divider=$di --result_path=tracking/tracking_results/${filename}_${d}_${s}_${di}.pkl --score_func=0
